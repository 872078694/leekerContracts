...
contract('LeekerToken', (accounts)=>{
    let tokenInstance;
    it('initalizes the contract with the correct value',()=>{
        return LeekerToken.deployed().then((instance)=>{
            tokenInstance = instance;
            return tokenInstance.name();
        }).then((name)=>{
            assert.equal(name,'LeekerSwap','has the correct name')
            return tokenInstance.symbol();
        }).then((symbol)=>{
            assert.equal(symbol,'LKS', 'has the correct symbol');
            return tokenInstance.standard();
        }).then((standard)=>{
            assert.equal(standard,'LeekerSwap v1.0','has the correct standard')
        })
    })
    ...
    it('transfers token ownership',()=>{
        return LeekerToken.deployed().then((instance)=>{
            ...
            assert.equal(balance.toNumber(),7500000,'deducts the amount from the sending accouts')
        })
    })
    ...
    it('approves tokens for delegated transfer', ()=>{
        return LeekerToken.deployed().then((instance)=>{
            ...
            assert.equal(receipt.logs.length,1,'triggers one event');
            assert.equal(receipt.logs[0].event, 'Approve' ,'should be the "Transfer" event');
    ...
    it('handle delegated token transfer', ()=>{
        let fromAccount;
        let toAccount;
        let spendingAccount;
        return LeekerToken.deployed().then((instance)=>{
            ...
            //in some version of solidity you have to transfer the message to string first then do the indexOf check
            ...
            assert.equal(balance.toNumber(),90,'deduct the amount from the sending amount');
            return tokenInstance.balanceOf(toAccount);
        }).then((balance)=>{
            assert.equal(balance.toNumber(),10,'add the amount to the receiving amount');
